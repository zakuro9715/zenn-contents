...
シェルスクリプトでは行ごとの操作を行うことが多いです。
これを反映したものが `...int` のような sequence 型であり、シェルスクリプトと同様にパイプでつなげて行ごとに処理する書き方ができます。
...
現状では分かりやすさを優先して標準出力をファイルに書き込むリダイレクトのみをサポートしており、`2>&1` などはサポートしていません。
複雑なリダイレクトを扱いたい場合は、後述のインラインシェルスクリプトを利用できます。
...
現時点ではまだまだ機能面が不十分なことは否めません。構造体、高階関数、匿名関数、if 式、パターンマッチなど、実装したい機能は多くあります。

また、配列は存在するものの、不完全な実装であり、うまく動かないパターンが多くあります。

標準ライブラリも全く足りておらず、またポータビリティを十分に確保できていない部分が多いです。

機能を増やすことを優先した結果、言語自体の設計もあまり優れていると言えない部分があります。

未踏の提案時点から一貫して、Cotowali は実際に使える言語になることを目標としています。
未踏の期間の勢いだけで実用的な言語を作るのは困難です。実用的なプログラミング言語を作るには、数年は必要と考えるべきでしょう。
未踏が終わってからも、燃え尽きることのないよう、無理せず地道に開発を行うことを意識しています。

さりとて未踏ではある程度の成果を出すことが必要であり、現在の実装は悩むよりもとりあえず作ることを優先しています。
深く考えずに作った部分も多く、実際に使ってみると良くない部分も見えてきます。
現状の実装はたたき台として割り切り、しばらくの間は必要なら大胆に変更を加えるつもりです。

当然のことながら、モチベーションがなくなったり時間が取れなくなったりして、結局辞めてしまう可能性も十分に考えられます。
そうならないように意識してはいますが、続けるという点において私は自分のことをそれほど信用していません。

しかし、理想というのは掲げ信じることが大事です。案外うまくいく可能性だってあります。理想を掲げ、しかし無理はせず、地道にやっていきたいと思います。
